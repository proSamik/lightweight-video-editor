...
ipcMain.handle('transcribe-audio', async (event, audioPath: string, model: string = 'base', language: string = 'auto') => {
  try {
...
ipcMain.handle('transcribe-audio-segments', async (event, audioPath: string, timelineSelections: any[], model: string = 'base', language: string = 'auto') => {
  try {
    const whisperService = WhisperService.getInstance();
